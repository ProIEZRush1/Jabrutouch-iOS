...
void FirestoreClient::Initialize(const User& user, const Settings& settings) {
  ...
  local_store_ = std::make_unique<LocalStore>(persistence_.get(),
  ...
  remote_store_ = std::make_unique<RemoteStore>(
  ...
  sync_engine_ =
      std::make_unique<SyncEngine>(local_store_.get(), remote_store_.get(),
                                    user, kMaxConcurrentLimboResolutions);

  event_manager_ = std::make_unique<EventManager>(sync_engine_.get());
