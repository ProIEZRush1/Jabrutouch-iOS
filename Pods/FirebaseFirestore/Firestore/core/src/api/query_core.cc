...
void Query::GetDocuments(Source source, QuerySnapshotListener&& callback) {
  ...
  auto listener = std::make_unique<ListenOnce>(source, std::move(callback));
...
std::unique_ptr<ListenerRegistration> Query::AddSnapshotListener(
  ...
  auto view_listener =
      std::make_unique<Converter>(this, std::move(user_listener));
  ...
  return std::make_unique<QueryListenerRegistration>(
