...
class HttpRequestsFactory {
    ...
    class func createGetLessonDonationRequest(lessonId: String, isGemara: Bool, token: String) -> URLRequest?{
        guard let baseUrl = URL(string: HttpRequestsFactory.baseUrlLink) else { return nil }
        let link = baseUrl.appendingPathComponent("lesson_donations").absoluteString
        let body: [String:Any] = ["lesson_id": lessonId, "is_gemara": isGemara]
        guard let url = self.createUrl(fromLink: link, urlParams: nil) else { return nil }
        let additionalHeaders: [String:String] = ["Authorization": "token \(token)"]
        
        let request = self.createRequest(url, method: .get, body: body, additionalHeaders: additionalHeaders)
        return request
    }
